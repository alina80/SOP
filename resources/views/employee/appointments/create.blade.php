...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("employee.appointments.store") }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="form-group {{ $errors->has('client') ? 'has-error' : '' }}">
                <label for="client">{{ trans('cruds.appointment.fields.client') }}</label>
                <input type="text" id="client" name="client" class="form-control" value="{{ old('client', isset($appointment) ? $appointment->client : '') }}">
            ...
            <div class="form-group {{ $errors->has('status_id') ? 'has-error' : '' }}" style="display: none">
                ...
                <input name="status_id" id="status" class="form-control select2" value="1">
